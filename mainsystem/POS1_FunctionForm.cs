...
namespace mainsystem
{
    ...
    public partial class POS1_FunctionForm : Form
    {
        ...
        // guard for formatting events
        private bool isFormattingChange = false;

        public POS1_FunctionForm()
        {
            InitializeComponent();
        }
        private void POS1_FunctionForm(object sender, EventArgs e)
        {
            isLoading = true;
        ...
        {
            // parse qty: return silently if invalid
            ...
            // parse price, strip thousand separators if present
            if (!double.TryParse(priceTxtbox.Text.Replace(",", ""), out double price) || price <= 0)
            {
                discountTxtbox.Text = "";
                discountedTxtbox.Text = "";
                return;
        ...
        private void SelectItem(string itemName, double price)
        {
            ...
            // Only compute if qty is already valid; otherwise clear outputs
            if (int.TryParse(qtyTxtbox.Text, out int qty) && qty > 0)
            {
                ComputeDiscounts();
            }
        ...
        private void pictureBox6_Click_1(object sender, EventArgs e)
        {
            ...
            // Removed accidental clearing of item/price that erased the selection
        }
